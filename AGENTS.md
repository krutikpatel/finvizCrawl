...
# Rebuild sentiment_summary.json from the log (no Codex call — pure aggregation)
...
3. **Single Codex call** — one `Codex --print` session covering all tickers that have new articles. Codex reads each ticker's `analysis_input.json` and appends scored records to `sentiment_log.jsonl`. Codex does **not** write `sentiment_summary.json`.
4. **Rebuild summaries (Python)** — `finzwiz rebuild-summary --ticker <TICKER>` for each analyzed ticker. Pure aggregation over `sentiment_log.jsonl`; no Codex call.

The sentiment analysis step uses Codex's own session (no `ANTHROPIC_API_KEY` required) via the `--print` flag of the `Codex` CLI at `/opt/homebrew/bin/Codex`.

## Sentiment analysis design

**Why Codex only sees new articles:**
Each article is scored in isolation — Codex reads the article text and decides bullish/bearish/neutral based on what the article says. Prior articles don't change how a new article reads; if an older event is still relevant, today's articles will reference it. Sending all historical articles to Codex wastes context window: with 100 articles per ticker × 9 tickers × ~25 KB each, a naive prompt balloons to ~23 MB, hitting Codex's session limit after 1–2 tickers.

**Why Python rebuilds the summary, not Codex:**
`sentiment_summary.json` is pure aggregation — counts, averages, grouping by date, sorting by score. It draws on *all* historical records in `sentiment_log.jsonl`, not just today's. Python (`finzwiz rebuild-summary`) reads the full log and recomputes the summary after each run. This means the summary always reflects the complete history, and Codex's job is reduced to scoring new text only.

**The split:**
- Codex's responsibility: read `analysis_input.json` (new articles, pre-filtered, text-truncated) → append scored records to `sentiment_log.jsonl`.
